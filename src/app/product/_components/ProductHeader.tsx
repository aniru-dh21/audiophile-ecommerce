...
export default function ProductHeader({
  ...
  headerData: ProductHeaderProps;
}) {
  const { name, description, price, productImageData, isNew, id, cartImage } =
    headerData;
  const { dispatch } = useCartData();
  const [quantity, setQuantity] = useState(1);
  const productImageRef = useRef<HTMLDivElement | null>(null);
  const [cartAnimationData, setCartAnimationData] =
    useState<CartAnimationProps>({
      top: 0,
      left: 0,
      width: 0,
      height: 0,
